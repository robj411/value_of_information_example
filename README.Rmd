...
```{r quantiles,echo=F}
quantiles <- t(apply(parameter_samples,2,quantile,c(0.05,0.5,0.95)))
kable(quantiles,digits=2)
```
...
for(j in 1:nrow(quantiles)){
  ...
  pollution_return <- pollution_calculation(params,1)
...
df$Parameter <- rownames(quantiles)
...
However, while the plot is useful for the parameters we might be able to learn on their own, $x_1$ and $x_2$, it is less useful for the parameters for the dose-response curve. This is because it's not the case that we could learn one parameter in particular. In addition, it's also not useful to take the 5th and 95th quantiles of $\beta$, say, while taking the 50th quantiles for the other parameters. This is because they have a joint density.
...
The input $x_3$ defines the relationship between PM2.5 and stroke. There exists a function, $f(\text{PM}2.5,x_3)$, that maps the PM2.5 concentation onto the relative risk (RR) of stroke, which is learnt from observational data. The function $f(\text{PM}2.5,x_3)$ defines a dose--response relationship, where the dose is the PM2.5 and the response is relative risk of stroke. The risk is relative to a PM2.5 value of 0, so the relative risk at PM2.5=0 is 1. We use values from Burnett et al. (2014, doi: 10.1289/ehp.1307049), where $x_3=\{\alpha,\beta,\gamma,\tau\}$, and

relative risk of stroke ($R$) = $f(\text{PM}2.5,x_3) = 1 + \alpha ( 1 - \exp(- \beta ( \text{PM}2.5 - \tau )^{\gamma} ) )$.

The uncertainty about the accuracy of the dose--response relationship is captured through the sampled values of the components of $x_3$. 

For our final computation, we also need the relative risk for the baseline, $R_0$:

$R_0 = 1 + (x_3-1)f(x_1,x_3)$.
